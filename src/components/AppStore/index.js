...
class AppStore extends Component {
  ...
  onSelectTab = tabId =>
    this.setState({activeTabId: tabId, isTabbed: true, isSearch: false})
  ...
  render() {
    ...
    let results
    if (isSearch) {
      results = initialAppList.filter(each =>
        each.appName.toLowerCase().includes(searchInput.toLowerCase()),
      )
    } else if (isTabbed) {
      results = initialAppList.filter(each => each.category === activeTabId)
    }
